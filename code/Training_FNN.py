...
for file in os.listdir(train_embs):
    temp_file = pickle.load(open(os.path.join(train_embs, file), "rb")).reset_index(drop=True)
    TRAIN_EMBS[file[:-2]] = [embeddings.shape[0], embeddings.shape[0] + temp_file.shape[0]]
    embeddings = embeddings.append(temp_file)

val_embs = WD / "ML_data" / "val_data" / "embeddings" / emb_name
for file in os.listdir(val_embs):
    temp_file = pickle.load(open(os.path.join(val_embs, file), "rb")).reset_index(drop=True)
    TRAIN_EMBS[file[:-2]] = [embeddings.shape[0], embeddings.shape[0] + temp_file.shape[0]]
    embeddings = embeddings.append(temp_file)

keys = list(TRAIN_EMBS.keys())
keys = np.array(keys)
np.random.seed(1)
...
for i in np.arange(5):
    ...
    for z in np.arange(5):
        if z != i:
            train_set = train_set.append(pd.DataFrame(keys[round(z*size):round((z+1)*size)]))
    val_set = val_set.append(pd.DataFrame(keys[round(i*size):round((i+1)*size)]))
    print(train_set.__len__())
    print(val_set.__len__())
    train_embs_cv = pd.DataFrame()
    val_embs_cv = pd.DataFrame()
    for t in train_set[0]:
        train_embs_cv = train_embs_cv.append(embeddings.iloc[(TRAIN_EMBS[t])[0]:(TRAIN_EMBS[t])[1]])
    for v in val_set[0]:
        val_embs_cv = val_embs_cv.append(embeddings.iloc[(TRAIN_EMBS[v])[0]:(TRAIN_EMBS[v])[1]])
    train_loader = torch.utils.data.DataLoader(dataset=torch.tensor(train_embs_cv.to_numpy()),
                                               batch_size=500,
                                               shuffle=False)
    model = FeedforwardNeuralNetModel(513, 3)
    optimizer = torch.optim.Adam(model.parameters())
    weight = (torch.Tensor([0, 365, 12478]))
